...
MAT_SLICES_PARAM = ("matrix_size,window_size",
                    [5, 10, 20, 50, 100], [2, 3, 10, 1, 20])
...
def test_subset_mat(matrix_size, window_size):
    ...
    coords = np.random.randint(0, matrix_size,
                               (np.random.randint(matrix_size // 2), 2))
    ...
    valid = np.where((coords[:, 0] > window_size / 2) &
